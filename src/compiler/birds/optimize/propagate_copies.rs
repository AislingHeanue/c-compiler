...
impl FlowGraph<BirdsInstructionNode, BirdsInstructionInfo> {
    ...
    fn replace_instruction(
        ...
        match instruction {
            ...
            | BirdsInstructionNode::DoubleToInt(ref mut src, _)
            | BirdsInstructionNode::DoubleToUint(ref mut src, _)
            | BirdsInstructionNode::IntToDouble(ref mut src, _)
            | BirdsInstructionNode::UintToDouble(ref mut src, _)
    ...
    fn transfer(
        ...
        for instruction in node.instructions.iter_mut() {
            ...
            match &instruction.0 {
                ...
                | BirdsInstructionNode::DoubleToInt(_, dst)
                | BirdsInstructionNode::DoubleToUint(_, dst)
                | BirdsInstructionNode::IntToDouble(_, dst)
                | BirdsInstructionNode::UintToDouble(_, dst)
